...
class GPTConfig:
    block_size: int = 1024 # set block size - defines maximum numbers of tokens
    vocab_size: int = 50304 # defines the total number of unique tokens
    n_layer: int = 12 # 
    n_head: int = 12 
    n_embd: int = 768
    dropout: float = 0.0
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        assert config.vocab_size is not None # checking if is not none
        assert config.block_size is not None # checking if it is not none
        self.config = config
        ...
        print('number of parameters :%.2f', (self.get_num_params()/1e6,))
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        print(f"loading weights from pretrained gpt: %s" % model_type)
        ...
        #init a huggingface/transfomer model
        ...
        # basically the openai checkpoints use a "Convi1D" module, but we only want to use a vanilla Linear
